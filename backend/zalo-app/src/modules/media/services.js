...
const MAX_FILE_SIZE = 550 * 1024; // tối đa 500KB
const MAX_TOTAL_SIZE = 550 * 1024;
...
const validateFile = (file) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
        return {
            isValid: false,
            message: `File "${file.originalname}" vượt quá giới hạn 550KB (hiện tại: ${(file.size/1024).toFixed(2)}KB)`,
            code: "FILE_TOO_LARGE"
        };
    }
...
const uploadSingleFile = (file) => {
    ...
    const key = `${timestamp}-${randomStr}-${file.originalname}`;
...
const uploadToS3 = async (files, conversationId) => {
    ...
    if (totalSize > MAX_TOTAL_SIZE) {
        throw {
            isValid: false,
            message: `Tổng dung lượng vượt quá 500KB. Hiện tại là ${(totalSize/1024).toFixed(2)}KB`,
